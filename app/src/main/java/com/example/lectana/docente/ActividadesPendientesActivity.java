...
public class ActividadesPendientesActivity extends AppCompatActivity {
    private static final String TAG = "ActividadesPendientes";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Log.d(TAG, "=== INICIANDO ActividadesPendientesActivity ===");
        
        try {
            setContentView(R.layout.activity_actividades_pendientes);
        ...
        try {
            ...
            Log.d(TAG, "ActividadesPendientesActivity inicializada completamente");
        } catch (Exception e) {
        ...
        try {
            adapter = new ActividadesAdapter(listaActividades, new ActividadesAdapter.OnActividadClickListener() {
                ...
                public void onActividadClick(Actividad actividad) {
                    ...
                    Intent intent = new Intent(ActividadesPendientesActivity.this, DetalleActividadActivity.class);
                ...
                public void onEditarClick(Actividad actividad) {
                    ...
                    Intent intent = new Intent(ActividadesPendientesActivity.this, CrearActividadActivityTest.class);
                    intent.putExtra("actividad_id", actividad.getId_actividad());
                ...
                public void onAsignarAulasClick(Actividad actividad) {
                    ...
                    Intent intent = new Intent(ActividadesPendientesActivity.this, AsignarAulasActivity.class);
                    intent.putExtra("actividad_id", actividad.getId_actividad());
                    startActivity(intent);
                }
            });
    ...
    private void cargarActividadesPendientes() {
        Log.d(TAG, "Iniciando carga de actividades pendientes...");
            ...
            actividadesRepository.getActividadesDocente(new ActividadesRepository.ActividadesCallback<List<Actividad>>() {
                ...
                public void onSuccess(List<Actividad> actividades) {
                    runOnUiThread(() -> {
                        ...
                        // Filtrar actividades pendientes (sin_corregir > 0)
                        List<Actividad> actividadesPendientes = filtrarPendientes(actividades);
                        actualizarDatosActividades(actividadesPendientes);
                    });
                }
                ...
                public void onError(String message) {
                    runOnUiThread(() -> {
                        ...
                        Toast.makeText(ActividadesPendientesActivity.this, "Error al cargar actividades: " + message, Toast.LENGTH_LONG).show();
                        mostrarMensajeSinActividades();
                    });
                }
            });
        } catch (Exception e) {
            Log.e(TAG, "Error en cargarActividadesPendientes", e);
            runOnUiThread(() -> {
                mostrarCargando(false);
    ...
    /**
     * Filtrar actividades pendientes de corrección
     * Actividades donde sin_corregir > 0
     */
    private List<Actividad> filtrarPendientes(List<Actividad> actividades) {
    ...
    private void actualizarContadorActividades() {
        ...
        if (textContadorActividades != null) {
            textContadorActividades.setText(total + " actividades pendientes");
        }
    }
                ...
                .setPositiveButton("Eliminar", (dialog, which) -> {
                    actividadesRepository.eliminarActividad(actividad.getId_actividad(), new ActividadesRepository.ActividadesCallback<Void>() {
                        ...
                        public void onSuccess(Void data) {
                            runOnUiThread(() -> {
                                Toast.makeText(ActividadesPendientesActivity.this, "Actividad eliminada exitosamente", Toast.LENGTH_SHORT).show();
                                cargarActividadesPendientes();
                            });
                        }
                        ...
                        public void onError(String message) {
                            runOnUiThread(() -> {
                                Toast.makeText(ActividadesPendientesActivity.this, "Error al eliminar actividad: " + message, Toast.LENGTH_LONG).show();
                            });
                        }
                    });
                })
